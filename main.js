...
classifier=ml5.imageClassifier('https://teachablemachine.withgoogle.com/models/2Kf7WAwGP/model.json',modelLoaded);
...
function gotResult(error,results){
    if(error){
        ...
        if(results[0].label=="Hand with Fingers Splayed"){
            document.getElementById("update_emoji1").innerHTML="🖐️";
        }
        if(results[0].label=="Call Me Hand"){
            document.getElementById("update_emoji1").innerHTML="🤙";
        }
        ...
        if(results[1].label=="Hand with Fingers Splayed"){
            document.getElementById("update_emoji2").innerHTML="🖐️";
        }
        if(results[1].label=="Call Me Hand"){
            document.getElementById("update_emoji2").innerHTML="🤙";
        }
